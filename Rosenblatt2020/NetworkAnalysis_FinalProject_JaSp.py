...
N = 50                     # Number of random networks = 5000
...
for i, G_true in enumerate(graphs[:5]):
    ...
    for G_observed, p in create_networkcopy_with_missing_nodes(G_true, P):
        ...
        # No immunization: Perform outbreak simulation in G_true without immunizing nodes to obtain mean and std of the 
        # outbreak size
        ...
        # to obtain mean and std of the outbreak size
        ...
        for name, centrality in centralities:
            ...
            # G_observed determined by the respective centrality measure to obtain mean and std of the outbreak size
...
for df in [data_degree, data_between, data_eigen, data_page]:
    ...
    # Print mean and std of both robustness measures over all true networks for each error level p
    print('\n', df.name)
